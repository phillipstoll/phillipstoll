...
namespace HarmonicAnalysisLib
{
    public class Manager
    {
        ...
        public void ProcessChords(PitchPattern[] chords)
        {
            ...
            //PrintGroups(GroupType.VerticalGroup);
            stopWatch.Stop();
            ...
            //PrintGroups(GroupType.HorizontalGroup);
            stopWatch.Stop();
            ...
            GroupMapLib.frameCounts = new(0, 0, 0);
        }
        ...
        public void GroupsToVertGroups(PitchPattern[] chords)
        {
            ...
            for (var iChord = 0; iChord < chords.Length; ++iChord)
            {
                ...
                // distinct Positions of Tones and Groups, projected into twelve 'shapes' restricted to strips y = [0..3], [1..4], [2..5], [3..6] and z = [0..1], [1..3], [2..4]
                var shapes = PitchConverter.ChordToShapes(chord);

                // the three positions of a VerticalGroup or HorizontalGroup
                var pitchTriplets = new List<PitchTriplet>();

                foreach (var shape in shapes)
                {
                    var fund = PitchConverter.GreatestLowerBound(shape);
                    var offset = VectorConverter.Normalize(fund);
                    ...
                    for (var i = 0; i < shape.Count - 1; i++)
                    {
                        ...
                        {
                            if (fund[1] == Math.Min(shape[i].Position[1], shape[j].Position[1]) &&
                                fund[2] == Math.Min(shape[i].Position[2], shape[j].Position[2]))
                            {
                                ...
                                if (delta1[1] <= 3 && delta1[2] <= 1 && delta2[1] <= 3 && delta2[2] <= 1)
                                {
                                    ...
                                    var pitchTriplet = new PitchTriplet(fund, shape[i], shape[j]);

                                    // sanity check
                                    if (pitchTriplets.Count(p => 
                                        p.Position.Equals(pitchTriplet.Position) &&
                                        p.Pitch1.Position.Equals(pitchTriplet.Pitch1.Position) &&
                                        p.Pitch2.Position.Equals(pitchTriplet.Pitch2.Position)) != 0) throw new Exception();

                                    // the same PitchTriplet will never be added twice
                                    list.Add(pitchTriplet);
                                    pitchTriplets.Add(pitchTriplet);
                                }
                            }
                        }
                    }
        ...
        public void GroupsToHorizGroups(PitchPattern[] chords)
        {
            ...
            for (var iChord = 0; iChord < chords.Length - 1; ++iChord)
            {
                ...
                var positions = PitchConverter.YZ_Iterator();
                foreach (var position in positions)
                {
                    var shape = PitchConverter.ChordToShapes(chord, position);
                    var shapeNext = PitchConverter.ChordToShapes(chordNext, position);
                    var fund = VectorConverter.CreateVector(
                        Math.Min(shape.Min(t => t.Position[0]), shapeNext.Min(t => t.Position[0])),
                    ...
                    {
                        for (var j = 0; j < shapeNext.Count; j++)
                        {
                            if (fund[1] == Math.Min(shape[i].Position[1], shapeNext[j].Position[1]) &&
                                fund[2] == Math.Min(shape[i].Position[2], shapeNext[j].Position[2]))
                            {
                                var delta = shape[i].Position - fund;
                                var deltaNext = shapeNext[j].Position - fund;
                                if (delta[1] <= 3 && delta[2] <= 1 && deltaNext[1] <= 3 && deltaNext[2] <= 1)
                                {
                                    if (shape[i] == shapeNext[j]) throw new Exception();
                                    var pitchTriplet = new PitchTriplet(fund, shape[i], shapeNext[j]);
        ...
        /// </summary>
        /// <param name="chords">chord progression</param>
        public void GroupsToHorizGroups_old(PitchPattern[] chords)
        {
            // When stored, the HorizGroup Positions are normalized, and the children Positions are normalized with respect to the parent Position
            ...
            for (var iChord = 0; iChord < chords.Length; ++iChord)
            {
                ...
                foreach (var frame in Groups.Where(g => g.ChordIndex == iChord && g.Type == GroupType.Frame ))
                {
                    foreach (var frameNext in Groups.Where(g => g.ChordIndex == iChord && g.Type == GroupType.Frame && g != frame))
                    {
                        ...
                        {
                            // to avoid VerticalFrames whose child Frames are the same but in different order
                            // standardize the order of the child Frames
                            if (!VectorConverter.IsPositiveVector(frameNextPos.Value - frame.Position))
                            {
                                continue;
                            }
        ...
        public PitchPattern GroupsToPitchPattern(int iChord)
        {
            ...
            var pitches = normalizedTo_z0.Select(p => new Pitch(VectorConverter.VectorToPitch(p), p[1], p)).Cast<IPitch>().ToList();
        ...
        {
            Debug.WriteLine("PrintGroups DISABLED");
        ...
        public void PrintGroupCounts()
        {
            ...
            Debug.WriteLine($"Tone:             {Groups.Where(g => g.Type == GroupType.Tone).Sum(g => g.Distance.CompoundWeighted)}");
            Debug.WriteLine($"Group:            {Groups.Count(g => g.Type == GroupType.Group)}");
            Debug.WriteLine($"Group:            {Groups.Where(g => g.Type == GroupType.Group).Sum(g => g.Distance.CompoundWeighted)}");
            ...
            Debug.WriteLine($"VerticalGroup:    {Groups.Where(g => g.Type == GroupType.VerticalGroup).Sum(g => g.Distance.CompoundWeighted)}");
            ...
            Debug.WriteLine($"Frame:            {Groups.Where(g => g.Type == GroupType.Frame).Sum(g => g.Distance.CompoundWeighted)}");
            Debug.WriteLine($"VerticalFrame:    {Groups.Count(g => g.Type == GroupType.VerticalFrame)}");
            Debug.WriteLine($"VerticalFrame:    {Groups.Where(g => g.Type == GroupType.VerticalFrame).Sum(g => g.Distance.CompoundWeighted)}");
            Debug.WriteLine($"HorizontalGroup:  {Groups.Count(g => g.Type == GroupType.HorizontalGroup)}");
            Debug.WriteLine($"HorizontalGroup:  {Groups.Where(g => g.Type == GroupType.HorizontalGroup).Sum(g => g.Distance.CompoundWeighted)}");
            ...
            Debug.WriteLine($"HorizontalFrame:  {Groups.Where(g => g.Type == GroupType.HorizontalFrame).Sum(g => g.Distance.CompoundWeighted)}");
        }
        ...
        #region Analytic
        IEnumerable<Group> OrderByHashCode(int iChord, GroupType groupType, Vector<int> transpose)
        {
            return Groups.Where(g => g.ChordIndex == iChord && g.Type == groupType)
                .OrderBy(group => {
                    var offset = VectorConverter.Normalize(group.Position + transpose);
                    var hash = group.GetPositionHashCode(transpose + offset);
                    return hash;
                });
        }
        private void PrintGroups(PitchPattern[] chords)
        {
            int hash;
            for (var iChord = 0; iChord < chords.Length; ++iChord)
            {
                var reference = Groups.Single(g => g.ChordIndex == iChord && g.Type == GroupType.Tone && g.Tone.PitchHeight == chords[iChord].Pitches[0].PitchHeight);
                var transpose = Vector<int>.Zero - reference.Position;

                //foreach (var group in Groups.Where(g => g.ChordIndex == iChord && g.Type == GroupType.Tone))
                //{
                //    var offset = VectorConverter.Normalize(group.Position + transpose);
                //    group.DebugWrite(0, transpose + offset);
                //    hash = group.GetPositionHashCode(transpose + offset);
                //}

                //foreach (var group in OrderByHashCode(iChord, GroupType.Group, transpose))
                //{
                //    var offset = VectorConverter.Normalize(group.Position + transpose);
                //    group.DebugWrite(0, transpose + offset);
                //}
                //foreach (var group in OrderByHashCode(iChord, GroupType.Frame, transpose))
                //{
                //    var offset = VectorConverter.Normalize(group.Position + transpose);
                //    group.DebugWrite(0, transpose + offset);
                //}
                foreach (var group in OrderByHashCode(iChord, GroupType.VerticalGroup, transpose))
                {
                    var offset = VectorConverter.Normalize(group.Position + transpose);
                    group.DebugWrite(0, transpose + offset);
                }
                //foreach (var group in OrderByHashCode(iChord, GroupType.VerticalFrame, transpose))
                //{
                //    var offset = VectorConverter.Normalize(group.Position + transpose);
                //    group.DebugWrite(0, transpose + offset);
                //}
                //foreach (var group in OrderByHashCode(iChord, GroupType.HorizontalGroup, transpose))
                //{
                //    var offset = VectorConverter.Normalize(group.Position + transpose);
                //    group.DebugWrite(0, transpose + offset);
                //}
                //foreach (var group in OrderByHashCode(iChord, GroupType.HorizontalFrame, transpose))
                //{
                //    var offset = VectorConverter.Normalize(group.Position + transpose);
                //    group.DebugWrite(0, transpose + offset);
                //}
        ...
        private double CountChordShapes()
        {
            double sum = 0;
            var count = 0;
            // public List<List<List<List<List<FrameStruct>>>>> ChordShapes /* [ChordIndex][VerticalHorizontal][DeltaIndex][z][y] */
            foreach (var chordIndex in ChordShapes)
            {
                foreach (var VerticalHorizontal in chordIndex)
                {
                    foreach (var DeltaIndex in VerticalHorizontal)
                    {
                        foreach (var z in DeltaIndex)
                        {
                            foreach (var y in z)
                            {
                                if (y.GroupMap != null)
                                {
                                    count++;
                                    sum += y.GroupMap.Distance.CompoundWeighted;
                                }
                                else
                                {
