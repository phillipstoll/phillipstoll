...
namespace HarmonicAnalysisLib
{
    ...
    public class Group : IEquatable<Group>
    {
        ...
        /// <summary>
        /// Whether all tones of a Group are within the bounds of this Frame
        /// </summary>
        /// <param name="group">the Group that is being test for whether it is within the bounds</param>
        /// <param name="offset">offset</param>
        /// <remarks>this Group is the Frame that has the bounds that are being tested</remarks>
        /// <returns></returns>
        public bool GroupWithinBounds(Group group, Vector<int> offset)
        {
            ValidateGroupType(GroupType.Frame);
            group.ValidateGroupType(GroupType.VerticalGroup, GroupType.Group, GroupType.Tone);

            Vector<int> lower, upper;
            FrameToBounds(out lower, out upper);
            //var offset = VectorConverter.Normalize(this.Position, group.Position);
            var inside = true;
            var tones = group.ListTonePositions(offset);
            foreach (var tuple in tones)
            {
                var p = tuple.Position;
                if (p[1] < lower[1] || upper[1] < p[1] || p[2] < lower[2] || upper[2] < p[2])
                {
                    inside = false;
                }
            }
        ...
        public List<Group> FrameToVerticalGroups_new(List<Group> groups)
        {
            ValidateGroupType(GroupType.Frame);
            var list = new List<Group>();

            var toneCount = this.ToneCount();
            var grps = groups.Where(g => g.ChordIndex == ChordIndex && g.Type == GroupType.Group &&
                (g.Children.Count == 2 || g.Children.Count == toneCount));
            foreach (var group in grps)
            {
                var offset = VectorConverter.Normalize(group.Position, this.Position);
                Debug.Assert(group.Children.All(c => c.Group.Type == GroupType.Tone));
                if (this.GroupWithinBounds(group, offset))
                {
                    list.Add(group);
                }
            }
            var vgrps = groups.Where(g => g.ChordIndex == ChordIndex && g.Type == GroupType.VerticalGroup);
            foreach (var group in vgrps)
            {
                var glb = PitchConverter.GreatestLowerBound(group.Children.Select(c => c.Position).ToList());
                var offset = VectorConverter.Normalize(glb, this.Position);
                if (this.GroupWithinBounds(group, offset))
                {
                    list.Add(group);
                }
            }
            //FrameStructLib.frameCount += list.Count;
        ...
        public List<Group> VerticalFrameToVerticalGroups_new(List<Group> groups)
        {
            var positions = this.ListTonePositions(Vector<int>.Zero);
            var s = string.Join(", ", positions.Select(p => VectorConverter.DebugFormatVector(p.Position)));

            positions = this.Children[0].Group.ListTonePositions(Vector<int>.Zero);
            var t = string.Join(", ", positions.Select(p => VectorConverter.DebugFormatVector(p.Position)));

            var o = this.Children[1].Offset() - this.Children[0].Offset();
            positions = this.Children[1].Group.ListTonePositions(Vector<int>.Zero);
            var u = string.Join(", ", positions.Select(p => VectorConverter.DebugFormatVector(p.Position)));
            ...
            var list = new List<Group>();
            var frame0 = this.Children[0].Group;
            var frame1 = this.Children[1].Group;
            ...
            foreach (var g in groups.Where(G => G.ChordIndex == ChordIndex && G.Type == GroupType.Group && G.Children.Count == toneCount))
            {
                ...
                if (frame0.GroupWithinBounds(g, VectorConverter.Normalize(g.Position, frame0.Position)) ||
                    frame1.GroupWithinBounds(g, VectorConverter.Normalize(g.Position, frame1.Position)))
                {
            ...
            // groups with 2 children
            foreach (var g in groups.Where(G => G.ChordIndex == ChordIndex && G.IsVerticalGroupOrGroup(2)))
            {
                Debug.Assert(g.Children.Count == 2);
                //Debug.Assert(g.Children[0].Offset() == Vector<int>.Zero);
                //Debug.Assert(this.Children[0].Offset() == Vector<int>.Zero);
                //Debug.Assert(g.Position == g.Children[0].Group.Position);
                //Debug.Assert(VectorConverter.PositionClassEqual(g.Position, g.Children[0].Group.Position));
                //Debug.Assert(this.Position == this.Children[0].Group.Position);
                //Debug.Assert(VectorConverter.PositionClassEqual(this.Position, this.Children[0].Group.Position));
                var offset = VectorConverter.Normalize(g.Children[0].Position, this.Children[0].Position);
                //var offset = VectorConverter.Normalize(g.Position, frame0.Position);

                if (frame0.GroupWithinBounds(g.Children[0].Group, offset))
                {
                    Debug.Assert(!list.Contains(g));
                    list.Add(g);
                    continue;
                ...
                //if (frame0.GroupWithinBounds(g.Children[0].Group, offset + g.Children[0].Offset()) &&
                //    frame1.GroupWithinBounds(g.Children[1].Group, offset + g.Children[1].Offset()))
                //{
                //    Debug.Assert(!list.Contains(g));
                //    list.Add(g);
                //    continue;
                //}
                //offset = VectorConverter.Normalize(g.Children[1].Group.Position + g.Children[1].Offset(), frame1.Position + this.Children[1].Offset());
                ...
                //offset = VectorConverter.Normalize(g.Children[0].Group.Position + g.Children[0].Offset(), frame1.Position + this.Children[1].Offset());

                //if (frame1.GroupWithinBounds(g.Children[0].Group, offset + g.Children[0].Offset()) &&
                //    frame0.GroupWithinBounds(g.Children[1].Group, offset + g.Children[1].Offset()))
                //{
                //    Debug.Assert(!list.Contains(g));
                //    list.Add(g);
                //    continue;
                //}
                //offset = VectorConverter.Normalize(g.Children[1].Group.Position + g.Children[1].Offset(), frame0.Position + this.Children[0].Offset());
        ...
        {
            ValidateGroupType(GroupType.HorizontalFrame);
            Vector<int> lower, upper, lowerNext, upperNext;
            FrameToBounds(out lower, out upper, out lowerNext, out upperNext);
        ...
        public bool Validate()
        {
            ...
            {
                case GroupType.Frame:
                    valid = valid && VectorConverter.PositionClassEqual(this.Position, this.Bounds[0]);
                    //valid = valid && this.Position[1] == this.Bounds[0][1] && this.Position[2] == this.Bounds[0][2];
            ...
            switch (this.Type)
            {
                ...
                case GroupType.Group:
                    break;
                case GroupType.VerticalGroup:
                    valid = valid && VectorConverter.IsNormalized(this.Children[0].Group.Position);
                    break;
                case GroupType.Frame:
                    break;
                case GroupType.HorizontalGroup:
            ...
            switch (this.Type)
            {
                ...
                case GroupType.Group:
                    break;
                case GroupType.VerticalGroup:
                    // children Positions are normalized with respect to the parent Position
                    break;
                case GroupType.Frame:
                    break;
                case GroupType.HorizontalGroup:
                    valid = valid && this.Children[0].Offset() == Vector<int>.Zero;
                    break;
                case GroupType.VerticalFrame:
                case GroupType.HorizontalFrame:
                    valid = valid && this.Children[0].Offset() == Vector<int>.Zero;
            ...
            switch (this.Type)
            {
                ...
                case GroupType.VerticalGroup:
                case GroupType.HorizontalGroup:
                    valid = valid && VectorConverter.PositionClassEqual(this.Position, PitchConverter.GreatestLowerBound(this.Children.Select(c => c.Position).ToList()));
                    // should never happen
                    if (!VectorConverter.PositionClassEqual(this.Position, PitchConverter.GreatestLowerBound(this.Children.Select(c => c.Position).ToList())))
                        break;
                    valid = valid && this.Position[0] <= this.Children.Min(c => c.Position[0]);
                    // should never happen
                    if (this.Position[0] > this.Children.Min(c => c.Position[0]))
                        break;
                    break;
                case GroupType.VerticalFrame:
                case GroupType.HorizontalFrame:
                    // should never happen
                    if (this.Position != this.Children[0].Position)
                        break;
                    // validate Children[0].Position
                    valid = valid && this.Position == this.Children[0].Position;
                    // validate Children[1].Position
                    valid = valid && Math.Abs(Children[1].Position[1] - Children[0].Position[1]) < 2;
                    valid = valid && Math.Abs(Children[1].Position[2] - Children[0].Position[2]) < 2;
                    break;
            }
            switch (this.Type)
            {
        ...
        public void DebugWrite_new(int depth, Vector<int> offset)
        {
            Debug.Write(new string(' ', 2 * depth));
            string offsetString = offset == Vector<int>.Zero ? "       " : "offset ";
            Vector<int> childOffset;
            var pos = Position + offset;
            var childPos = Children.Select(c => c.Position + offset).ToList();
            ...
            switch (this.Type)
            {
                case GroupType.Tone:
                    ...
                    {
                        // TODO: unccoment
                        //Debug.WriteLine($"{Type,-15} {ChordIndex,2} : {pos[0],3} {pos[1],2} {pos[2],2} : {Tone.PitchHeight,3} {Tone.FifthHeight,2}");
                        Debug.WriteLine($"{Type,-15} {ChordIndex,2} : {pos[0],3} {pos[1],2} {pos[2],2}");
                    }
                    break;
                case GroupType.Group:
                    Debug.WriteLine($"{Type,-15} {ChordIndex,2} : {pos[0],3} {pos[1],2} {pos[2],2}");
                ...
                case GroupType.Frame:
                    ...
                    Debug.WriteLine($"{Type,-15} {ChordIndex,2} : {VectorConverter.DebugFormatVector(Bounds[0] + offset)} : {VectorConverter.DebugFormatVector(Bounds[1] + offset)}");
                    //foreach (var child in Children)
                    //{
                    //    child.Group.DebugWrite(depth + 1, child.Offset() + offset);
                    //}
                ...
                case GroupType.HorizontalFrame:
                    ...
                    Debug.Write($"{VectorConverter.DebugFormatVector(Children[0].Group.Bounds[0] + offset)} : {VectorConverter.DebugFormatVector(Children[0].Group.Bounds[1] + offset)} : {VectorConverter.DebugFormatVector(Children[1].Group.Bounds[0] + childOffset + offset)} : {VectorConverter.DebugFormatVector(Children[1].Group.Bounds[1] + childOffset + offset)}");
        ...
        public void DebugWrite(int depth, Vector<int> offset)
        {
            ...
            switch (this.Type)
            {
                case GroupType.Tone:
                    ...
                    break;
                case GroupType.Group:
                    //Debug.WriteLine($"{Type,-15} {ChordIndex,2} : {pos[0],3} {pos[1],2} {pos[2],2} : {offsetString} {Distance.CompoundWeighted}");
        ...
        #region Unused code
        public int GetPositionHashCode(Vector<int> offset)
        {
            var hash = 0;
            var pos = Position + offset;
            hash += Math.Abs(pos[0]) + 3 * Math.Abs(pos[1]) + 5 * Math.Abs(pos[2]);
            foreach (var c in Children)
            {
                var childPos = c.Offset() + offset;
                hash += c.Group.GetPositionHashCode(childPos);
            }
            switch (Type)
            {
                case GroupType.Frame:
                    ...
                    {
                        var boundPos = bounds + offset;
                        hash += Math.Abs(boundPos[0]) + 3 * Math.Abs(boundPos[1]) + 5 * Math.Abs(boundPos[2]);
                    }
                    break;
                default:
                    break;
